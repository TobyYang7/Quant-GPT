...
def prepare_k_items_announcement_with_time_restriction(stock_prices: pd.DataFrame, unique_stocks_list: list, k: int, len_reference_list, reference_list, t_limit: int = None, token_limit: int = 3.2*1e4, postfix: str = None, with_macro=False):
    ...
    for stock in tqdm(unique_stocks_list):
        ...
        while j < len(news):
            ...
            while sum(token_len) > token_limit - new_added:
                working_list.pop(0)
                news_times.pop(0)
                token_len.pop(0)
                if len(news_times) == 0:
...
if __name__ == '__main__':
    ...
    print(stock_prices)
    log_r = prepare_k_days_return(stock_prices, 3)
    ...
    data = prepare_k_items_announcement_with_time_restriction(log_r, unique_stocks_list, 10, "summary_token_len", "summary", 30, 1.9*1e3, '_summary_new', False)
# %%
    table = pa.Table.from_pandas(data)
    pq.write_table(table, '../data/announcement/processed_LLM_data_3_10_30_2k_summary.parquet')
# %%
