...
def prepare_k_items_announcement_with_time_restriction(stock_prices: pd.DataFrame, unique_stocks_list: list, k: int, len_reference_list, reference_list, t_limit: int = None, token_limit: int = 3.2*1e4, postfix: str = None):
    combined_info = []
    stock_symbols = []
    log_rs = []
    trading_time = []
    ...
    for stock in tqdm(unique_stocks_list):
        ...
        while j < len(news):
            ...
            if len(working_list) >= k:
                ...
                news_times.pop(0)
                token_len.pop(0)
            while sum(token_len) > token_limit - news.loc[:, len_reference_list][j]:
                working_list.pop(0)
                news_times.pop(0)
                token_len.pop(0)
                if len(news_times) == 0:
            ...
            assert (calculate_token_length(combined_info[-1]) <= token_limit + 100)
...
if __name__ == '__main__':
    ...
    data = prepare_k_items_announcement_with_time_restriction(log_r, unique_stocks_list, 10, "summary_token_len", "summary", 30, 8*1e3, '_summary_new')
# %%
    table = pa.Table.from_pandas(data)
    pq.write_table(table, '../data/announcement/processed_LLM_data_1_10_30_8k_summary_new.parquet')
# %%
