...
def load_weights(input_dir: str) -> Dict[str, torch.Tensor]:
    ...
    for filepath in os.listdir(input_dir):
        ...
        if os.path.isfile(full_path):
            ...
            elif filepath == "pytorch_model.bin":
                model_weights = torch.load(full_path, map_location="cpu")
                for key, value in model_weights.items():
                    # Adjust the key names to fit the conversion logic
                    qwen_state_dict[key] = value
    return qwen_state_dict


def convert_weights(qwen_state_dict: Dict[str, torch.Tensor], torch_dtype: torch.dtype) -> Dict[str, torch.Tensor]:
    ...
    return llama2_state_dict


def save_model_weights(llama2_state_dict: Dict[str, torch.Tensor], output_dir: str, shard_size: str, save_safetensors: bool):
...
def llamafy_qwen(input_dir: str, output_dir: str, shard_size: Optional[str] = "2GB", save_safetensors: Optional[bool] = False):
    try:
        os.makedirs(output_dir, exist_ok=True)
    except Exception as e:
        raise print("Output dir already exists", e)

    qwen_state_dict = load_weights(input_dir)
    torch_dtype = next(iter(qwen_state_dict.values())).dtype
    llama2_state_dict = convert_weights(qwen_state_dict, torch_dtype)
    save_model_weights(llama2_state_dict, output_dir, shard_size, save_safetensors)
    save_config(input_dir, output_dir, str(torch_dtype).replace("torch.", ""))
